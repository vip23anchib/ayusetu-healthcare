...
import { Calendar, Clock, CalendarDays, CheckCircle, Trash2 } from 'lucide-react';
...
const DoctorDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-primary-800 via-primary-700 to-navy-800 rounded-3xl p-8 text-white shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-800/90 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 elevated-card flex items-center space-x-4">
          <div className="h-10 w-10 rounded-xl bg-primary-50 dark:bg-primary-950/80 flex items-center justify-center text-primary-700 dark:text-primary-300 shrink-0">
            <Clock className="h-5 w-5" />
          </div>
          <div>
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase block tracking-wider font-semibold">Today's Confirmed</span>
            <span className="text-lg font-extrabold text-slate-800 dark:text-slate-100">{confirmedCount}</span>
          </div>
        </div>

        <div className="bg-white dark:bg-slate-800/90 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 elevated-card flex items-center space-x-4">
          <div className="h-10 w-10 rounded-xl bg-teal-50 dark:bg-teal-950/80 flex items-center justify-center text-teal-600 dark:text-teal-400 shrink-0">
            <CheckCircle className="h-5 w-5" />
          </div>
          <div>
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase block tracking-wider font-semibold">Completed Consults</span>
            <span className="text-lg font-extrabold text-slate-800 dark:text-slate-100">{completedCount}</span>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2">
            <Card title="Patient Consultation Schedule" subtitle="Check raw symptoms and review triage analysis details before launching consultations.">
              {appointments.length === 0 ? (
                <p className="text-xs text-slate-500 dark:text-slate-400 py-8 text-center font-medium">No appointments scheduled.</p>
              ) : (
                <div className="space-y-4">
                  {appointments.map((appt) => (
                    <div
                      key={appt.id}
                      className="border border-slate-200 dark:border-slate-700 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-slate-50/50 dark:hover:bg-slate-700/40 transition-colors"
                    >
                      <div className="space-y-1">
                        <div className="flex items-center space-x-2">
                          <span className="font-bold text-slate-900 dark:text-slate-100 text-sm">{appt.patient.name}</span>
                          <span className="text-slate-300 dark:text-slate-600">|</span>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold">ID #{appt.id}</span>
                        </div>
                        <div className="flex items-center space-x-2.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
                        ...
                        {appt.symptoms && (
                          <div className="mt-3 text-xs bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700/80 max-w-lg font-medium text-slate-700 dark:text-slate-300">
                            <span className="font-bold text-slate-900 dark:text-slate-100 block mb-1">Chief Complaint:</span>
                            <p className="italic">"{appt.symptoms.symptoms_text}"</p>
                          </div>
                        )}
                      </div>

                      <div className="flex items-center space-x-3 w-full sm:w-auto shrink-0 justify-between sm:justify-end border-t sm:border-t-0 border-slate-100 dark:border-slate-700 pt-3 sm:pt-0">
                        {appt.pre_visit_summary && (
                          <div className="flex flex-col items-end shrink-0">
                            <span className="text-[9px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-0.5">Triage</span>
          ...
          <div className="space-y-6">
            ...
            <Card title="Mark Doctor Leave" subtitle="Select leave dates. Configured leaves cancel all pending bookings.">
              <form onSubmit={handleAddLeave} className="space-y-4">
                <div>
                  <label htmlFor="leaveDate" className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Leave Date</label>
                  <input
                    type="date"
                    id="leaveDate"
                    required
                ...
                <div>
                  <label htmlFor="leaveReason" className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Reason / Description</label>
                  <input
                    type="text"
                    id="leaveReason"
                    value={leaveReason}
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-700/60 max-h-60 overflow-y-auto">
                  {doctorProfile.leaves.map((leave) => (
                    <div key={leave.id} className="py-3 flex justify-between items-center text-xs">
                      <div>
                        <span className="font-bold text-slate-800 dark:text-slate-200">{leave.leave_date}</span>
                        {leave.reason && <p className="text-slate-500 dark:text-slate-400 mt-0.5 font-medium">{leave.reason}</p>}
                      </div>
                      <button
                        onClick={() => handleDeleteLeave(leave.leave_date)}
                        className="text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 p-2 rounded-xl transition-all cursor-pointer"
                      >
